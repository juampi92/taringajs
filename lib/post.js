...
module.exports = (function() {
  ...
  function Post(taringa) {
    ...
    /**
   * Returns the top posts: hoy, ayer y semana
   * @method topPosts
   * @param  {Number} amount (max = 14)
   * @return {Deferred}
   */
  Post.prototype.topPosts = function(amount) {
    var deferred = Defer();
    amount = amount || 15;

    this.taringa.request('http://www.taringa.net/ajax/top-posts.php?key=' + this.taringa.user_key + '&amount=' + amount,
      requestHandler.DataBased(this.taringa, deferred, 'User::TopPosts'));
