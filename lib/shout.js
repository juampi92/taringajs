...
module.exports = (function() {
  ...
  Shout.prototype.like = function(id, unlike) {
    ...
    }, requestHandler.RequestBased(this.taringa, deferred, 'Shout::' + (unlike ? 'unlike' : 'like')));
   ...
   * @method fav
   * @param  {Number} id
   * @param {Boolean} remove Remove from favs(default = false)
   * @return {Deferred}
   */
  Shout.prototype.fav = function(id, remove) {
    var deferred = Defer(),
      self = this;

    this.taringa.request.post('http://www.taringa.net/serv/shout/favourite/' + (remove ? 'remove' : 'add'), {
      form: {
        key: self.taringa.user_key,
        object_id: id
      }
    }, requestHandler.RequestBased(this.taringa, deferred, 'Shout::' + (remove ? 'remove' : 'add') + '_fav'));
   ...
   * @return {Deferred}
   */
  Shout.prototype.unfav = function(id) {
    return this.fav(id, true);
  };
   ...
   * @return {Deferred}
   */
  Shout.prototype.get = function(shout_id) {
    var deferred = Defer(),
      self = this;

    this.taringa.request('http://api.taringa.net/shout/view/' + shout_id,
      requestHandler.DataBased(this.taringa, deferred, 'Shout::get'));
   ...
   * @return {Deferred}
   */
  Shout.prototype.delete = function(shout_id) {
    var deferred = Defer(),
      self = this;

    this.taringa.request.post('http://www.taringa.net/serv/shout/delete', {
      form: {
        key: self.taringa.user_key,
        object_id: id
      }
    }, requestHandler.RequestBased(this.taringa, deferred, 'Shout::delete'));
   ...
   * @return {Deferred}
   */
  Shout.prototype.getCommentReplies = function(comment_id, obj_id, obj_owner, obj_type) {
    var deferred = Defer(),
      self = this;
