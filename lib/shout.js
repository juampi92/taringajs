...
module.exports = (function() {
  ...
  Shout.prototype.like = function(id, unlike) {
    ...
    this.taringa.request.post('http://www.taringa.net/serv/shout/' + (unlike ? 'un' : '') + 'like', {
      form: {
        key: self.taringa.user_key,
        object_id: id
      }
      ...
      if (!error) {
        ...
        deferred.resolve(response.statusCode === 200, response.statusCode, body);

        if (response.statusCode === 200) {
          return self.taringa.log('Shout::like -> Success on liking ' + id);
        } else {
          return self.taringa.error('Shout::like -> ID: ' + id + ' error: ' + response.statusCode);
        }

      } else {
        deferred.reject(error);
        return self.taringa.error('Shout::like -> Request was not succesful');
      }
    });
  ...
  Shout.prototype.get = function(shout_id, callback) {
    ...
    this.taringa.request('http://api.taringa.net/shout/view/' + shout_id, function(error, response, body) {
      var data;
      if (!error && response.statusCode === 200) {
        data = JSON.parse(body);
