...
module.exports = (function() {
  ...
  /**
   * Returns the top weekly users, grouped by followers and points
   * @method topWeeklyUsers
   * @param  {Number} amount (max = 15)
   * @return {Deferred}
   */
  User.prototype.topWeeklyUsers = function(amount) {
    var deferred = Defer();
    amount = amount || 15;

    this.taringa.request('http://www.taringa.net/ajax/top-usuarios.php?key=' + this.taringa.user_key + '&amount=' + amount,
      requestHandler.DataBased(this.taringa, deferred, 'User::TopWeeklyUsers'));
